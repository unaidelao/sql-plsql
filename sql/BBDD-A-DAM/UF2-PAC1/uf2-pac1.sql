...
-- versión 2.0: realizadas mejoras al declarar los CONSTRAINT
...
(
  ...
  CONSTRAINT PK_HOTEL PRIMARY KEY (codigo_hotel)
);
...
(
  ...
  medio VARCHAR(20),
  hotel CHAR(4),
  CONSTRAINT FK_SALAS FOREIGN KEY (hotel) REFERENCES HOTEL (codigo_hotel),
  CONSTRAINT PK_SALAS PRIMARY KEY (codigo_salas)
);
...
(
  ...
  pais CHAR(4),
  CONSTRAINT FK_PARTICIPANTES FOREIGN KEY (pais) REFERENCES PAIS (codigo_pais),
  CONSTRAINT PK_PARTICIPANTES PRIMARY KEY (num_asoc)
);
...
(
  ...
  fecha_in DATE,
  fecha_out DATE,
  CONSTRAINT FK_ALOJAR_1 FOREIGN KEY (num_asoc) REFERENCES PARTICIPANTES (num_asoc),
  CONSTRAINT FK_ALOJAR_2 FOREIGN KEY (hotel) REFERENCES HOTEL (codigo_hotel),
  CONSTRAINT PK_ALOJAR PRIMARY KEY (num_asoc,hotel)
);
...
(
  num_asoc CHAR(3) NOT NULL,
  nivel INT UNSIGNED,
  CONSTRAINT FK_JUGADORES FOREIGN KEY (num_asoc) REFERENCES PARTICIPANTES (num_asoc),
  CONSTRAINT PK_JUGADORES PRIMARY KEY (num_asoc)
);

CREATE TABLE IF NOT EXISTS ARBITROS
(
  num_asoc CHAR(3) NOT NULL,
  CONSTRAINT FK_ARBITROS FOREIGN KEY (num_asoc) REFERENCES PARTICIPANTES (num_asoc),
  CONSTRAINT PK_ARBITROS PRIMARY KEY (num_asoc)
);
...
(
  ...
  arbitro cHAR(3),
  codigo_salas CHAR(4),
  CONSTRAINT FK_PARTIDA_1 FOREIGN KEY (arbitro) REFERENCES ARBITROS (num_asoc),
  CONSTRAINT FK_PARTIDA_2 FOREIGN KEY (codigo_salas) REFERENCES SALAS (codigo_salas),
  CONSTRAINT PK_PARTIDA PRIMARY KEY (codigo_partida)
);

CREATE TABLE IF NOT EXISTS REPRESENTA
(
  pais_1 CHAR(4) NOT NULL,
  pais_2 CHAR(4) NOT NULL,
  CONSTRAINT FK_REPRESENTA_1 FOREIGN KEY (pais_1) REFERENCES PAIS (codigo_pais),
  CONSTRAINT FK_REPRESENTA_2 FOREIGN KEY (pais_2) REFERENCES PAIS (codigo_pais),
  CONSTRAINT PK_REPRESENTA PRIMARY KEY (pais_1, pais_2)
);
...
(
  ...
  CONSTRAINT FK_JUGAR_1 FOREIGN KEY (num_asoc) REFERENCES PARTICIPANTES (num_asoc),
  CONSTRAINT FK_JUGAR_2 FOREIGN KEY (codigo_partida) REFERENCES PARTIDA (codigo_partida),
  CONSTRAINT PK_JUGAR PRIMARY KEY (num_asoc, codigo_partida)
);
 ...
 */
 ...
 INSERT INTO PARTIDA VALUES ('par04', '005', 's005');
